...
from evaluate_models_utils_case import evaluate_model_link_prediction
...
from models.case_study import DyConNet
...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        elif args.model_name == 'DyConNet':
            # four floats that represent the mean and standard deviation of source and destination node time shifts in the training data, which is used for JODIE
            src_node_mean_time_shift, src_node_std_time_shift, dst_node_mean_time_shift_dst, dst_node_std_time_shift = \
                compute_src_dst_node_time_shifts(train_data.src_node_ids, train_data.dst_node_ids,
                                                 train_data.node_interact_times)
            dynamic_backbone = DyConNet(node_raw_features=node_raw_features, edge_raw_features=edge_raw_features,
                                        neighbor_sampler=train_neighbor_sampler,
                                        time_feat_dim=args.time_feat_dim, model_name=args.model_name,
                                        num_layers=args.num_layers, num_heads=args.num_heads,
                                        dropout=args.dropout, src_node_mean_time_shift=src_node_mean_time_shift,
                                        ...
                                        dst_node_mean_time_shift_dst=dst_node_mean_time_shift_dst,
                                        dst_node_std_time_shift=dst_node_std_time_shift, device=args.device,
                                        device_id=args.gpu, window_size=args.window_size,
                                        neighbor_order_range=args.neighbor_order_range,
                                        order1_neighbor_nums=args.order1_neighbor_nums,
                                        order2_neighbor_nums=args.order2_neighbor_nums,
                                        order3_neighbor_nums=args.order3_neighbor_nums,
                                        relation_order=args.relation_order, bipartite=args.bi,
                                        matrix_on_gpu=args.matrix_on_gpu,
                                        datasets=args.dataset_name)
        else:
            raise ValueError(f"Wrong value for model_name {args.model_name}!")

        if args.model_name == 'DyConNet':
            link_predictor = MergeLayer(input_dim1=node_raw_features.shape[1]*2, input_dim2=node_raw_features.shape[1]*2,
        ...
        for epoch in range(args.num_epochs):
            ...
            if args.model_name in ['DyConNet']:
                # training, only use training graph
                # model[0].set_neighbor_sampler(train_neighbor_sampler)
                # reinitialize memory of memory-based models at the start of each epoch
                model[0].memory_bank.__init_memory_bank__()
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                batch_end = min(batch_start + args.batch_size, train_data.num_interactions)
                # print(f'batch id is {batch_idx}.')
                # print(f'batch start at {batch_start}, batch end at {batch_end}.')
                assert np.array_equal(batch_src_node_ids, train_data.src_node_ids[
                                                          batch_start:batch_end]), 'dataloader computes differently as manual counts'
                ...
                elif args.model_name in ['DyConNet']:
                    time1 = time.time()
                    # # Parameter initialization
                    # del_batch_src_node_ids, del_batch_dst_node_ids = None, None
                    # slide_window = False
                    # # Compute the edges to be removed from the adjacency matrix
                    # # Set the batch to be removed when the window slides
                    # if batch_start >= args.window_size:
                    #     slide_window = True
                    #     # Compute the starting and ending indices of the edges to be removed in the current batch
                    #     del_batch_start = batch_start - args.window_size
                    #     del_batch_end = del_batch_start + args.batch_size
                    #     # Read the source and destination node IDs of the edges to be removed in the current batch
                    #     del_batch_src_node_ids = train_data.src_node_ids[del_batch_start:del_batch_end]
                    #     del_batch_dst_node_ids = train_data.dst_node_ids[del_batch_start:del_batch_end]
                ...
                if args.model_name in ['JODIE', 'DyRep', 'TGN', 'DyConNet']:
            ...
            if args.model_name in ['DyConNet']:
                train_backup_matrix = model[0].backup_matrix()
            ...
            if args.model_name in ['DyConNet']:
                val_backup_matrix = model[0].backup_matrix()
                model[0].reload_matrix(train_backup_matrix)
            ...
            if args.model_name in ['DyConNet']:
                model[0].reload_matrix(val_backup_matrix)
            ...
            logger.info(
                f'Epoch: {epoch + 1}, time: {time.time() - epoch_time}')
        ...
        if args.model_name not in ['JODIE', 'DyRep', 'TGN', 'DyConNet']:
            val_losses, val_metrics = evaluate_model_link_prediction(model_name=args.model_name,
                                                                     model=model,
                                                                     neighbor_sampler=full_neighbor_sampler,
                                                                     evaluate_idx_data_loader=val_idx_data_loader,

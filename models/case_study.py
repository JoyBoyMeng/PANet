...
from models.modules_case import TimeEncoder, MergeLayer, TemporalMultiHeadAttention, SpatialMultiHeadAttention
...
class DyConNet(torch.nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 ...
                 bipartite: bool = False, matrix_on_gpu: bool = True, datasets: str = ''):
        """
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
                                                 ...
                                                 num_neighbors: int = 20, epoch=1000, stage='fuck'):
        """
        ...
        node_spatial_embeddings_t = self.spatial_embedding_module.compute_node_temporal_embeddings(
            node_memories=updated_node_memories,
            node_ids=node_ids,
            neighbor_node_ids=neighbors_1,
            neighbor_space_features=neighbor_space_features,
            current_layer_num=1)
        node_spatial_embeddings = node_spatial_embeddings_t[0]
        if edges_are_positive:
            if self.datasets == 'yelp':
                tid = 372
            elif self.datasets == 'wikipedia': # 18
                tid = 1088
            elif self.datasets == 'aps': # 18
                tid = 34
            indices = np.where(src_node_ids == tid)[0]
            if indices.size != 0:
                indice = indices[0]
                score = node_spatial_embeddings_t[1]  # (400, 2, 50)
                score = torch.mean(score, dim=1)  # （400, 50）
                score = score[indice]
                if self.datasets == 'yelp':
                    score = torch.sigmoid(score * 1)
                elif self.datasets == 'wikipedia':
                    score = torch.sigmoid(score)
                elif self.datasets == 'aps':
                    score = torch.sigmoid(score)
                id_s = src_node_ids[indice]
                id_e = dst_node_ids[indice]
                od_edge = edge_ids[indice]

                filename = f"/autodl-fs/dyg/DyGLib/case/{self.datasets}_epoch_{epoch}_{stage}.txt"

                if not os.path.exists(filename):
                    with open(filename, 'w') as f:
                        f.write('id is :\n')
                        f.write(f'{id_s} {id_e} {od_edge}\n')
                        f.write('neighbor is:\n')
                        f.write(f'{neighbors_1[indice]}\n')
                        f.write('[1,0] order is:\n')
                        f.write(f'{src1_dst0_co_values[indice]}\n')
                        f.write('[1,2] order is:\n')
                        f.write(f'{src1_dst2_co_values[indice]}\n')
                        f.write('attention score is:\n')
                        f.write(f'{score}\n')
                else:
                ...
                # print('id is :')
                # print(id_s, id_e, od_edge)
                # print('neighbor is:')
                # print(neighbors_1[indice])
                # print('[1,0] order is:')
                # print(src1_dst0_co_values[indice]) # (400, 50, 2) --> (50, 2)
                # print('[1,2] order is:')
                # print(src1_dst2_co_values[indice])  # (400, 50, 2) --> (50, 2)
                # print('attention score is:')
                # print(score) # (400, 50) --> (50)
...
class SpatialAttentionEmbedding(nn.Module):
    ...
    def compute_node_temporal_embeddings(self, node_memories: torch.Tensor, node_ids: np.ndarray,
        ...
        output, score = self.temporal_conv_layers[current_layer_num - 1](node_features=node_features,

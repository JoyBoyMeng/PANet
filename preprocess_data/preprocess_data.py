...
def preprocess_new(dataset_name: str):
    """
    read the original data file and return the DataFrame that has columns ['u', 'i', 'ts', 'label', 'idx']
    :param dataset_name: str, dataset name
    :return:
    """
    k = 0
    if 'aminer' in dataset_name or 'aps' in dataset_name:
        k = 1
    u_list, i_list, ts_list, label_list = [], [], [], []
    feat_l = []
    idx_list = []
...
def preprocess_data_new(dataset_name: str, bipartite: bool = True, node_feat_dim: int = 172):
    """
    preprocess the data
    :param dataset_name: str, dataset name
    :param bipartite: boolean, whether the graph is bipartite or not
    ...
    df, edge_feats = preprocess_new(PATH)
    # new_df = reindex(df, bipartite)
...
parser.add_argument('--dataset_name', type=str,
                    choices=['wikipedia', 'reddit', 'mooc', 'lastfm', 'myket', 'enron', 'SocialEvo', 'uci',
                             'Flights', 'CanParl', 'USLegis', 'UNtrade', 'UNvote', 'Contacts', 'yelp', 'aminer', 'aps', 'pet', 'game', 'clothing'],
...
if args.dataset_name in ['enron', 'SocialEvo', 'uci']:
    ...
    elif args.dataset_name in ['yelp', 'aps', 'aminer', 'pet', 'game', 'clothing']:
        preprocess_data_new(dataset_name=args.dataset_name, bipartite=True, node_feat_dim=args.node_feat_dim)
    else:

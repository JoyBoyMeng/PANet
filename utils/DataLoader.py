...
def get_link_prediction_data(dataset_name: str, val_ratio: float, test_ratio: float):
    ...
    edge_file_path = './processed_data/{}/ml_{}.npy'.format(dataset_name, dataset_name)
    node_file_path = './processed_data/{}/ml_{}_node.npy'.format(dataset_name, dataset_name)
    ...
    if os.path.exists(edge_file_path):
        edge_raw_features = np.load(edge_file_path)
    else:
        edge_raw_features = np.zeros((full_data.num_interactions+1, EDGE_FEAT_DIM))
    if os.path.exists(node_file_path):
        node_raw_features = np.load(node_file_path)
    else:
        node_raw_features = np.zeros((full_data.num_unique_nodes+1, NODE_FEAT_DIM))
